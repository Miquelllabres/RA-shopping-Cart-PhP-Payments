...
export default class ShoppingCart{
   ...
   $("#content").html(output);
        ...
        for (let sku in sessionStorage){
            ...
            for (let product in products){
                ...
                if (products[currentProduct].sku.toString() == currentSku ){
                    ...
                    // total.setAttribute("data-sku", `${actualProduct.sku}`);
                    let totalTextNode = document.createTextNode("Total: " + "$" + `${subTotal}`);
                    ...
                    // console.log("i am in the appending view");
                ...
                let CartTotal = document.getElementsByClassName("subtotal");
                    ...
                    // console.log(Total);
                    let addTotal = `<p  class='greentext'>Total Price: $  ${Total}</p><input type="hidden" name="TOTAL" id="TOTAL" value="${Total}"></input>`;
           ...
           let removedItem = sessionStorage.getItem(sku);
            ...
            for (let i=0;i < CartTotal.length ;i++) {
             ...
             Total += subtotals;
                    ...
                    // console.log(Total);
            let addTotal = `<p id="TOTAL" class='greentext'>Total Price: $ ${Total}</p>`;
            $('#totalamount').html(addTotal);
        ...
        for (let p=0; p<sessionStorage.length; p++){
         ...
         for (let sku in sessionStorage){
                ...
                if (products[currentProduct].sku.toString() == currentSku ){
                    ...
                    let addTotal = `<p class='greentext'>Total: $ ${Total}</p>`;

                    // console.log(Total);
                    let Cart = document.getElementsByClassName("subtotal");
                    // console.log(Cart);

                


                    $('.subtotal').html(addTotal);
    ...
    $('#payForm').toggle();
        ...
        let collectDataMiddleGuy = function(){
            console.log(thisFile);
            return function(e){
                console.log('hello');
                var $form = $('#payment-form');
                console.log(thisFile.stripeResponseHandler);
                // **** START ****
                $form.submit(thisFile,function(event) {
                // Disable the submit button to prevent repeated clicks:
                $form.find('.submit').prop('disabled', true);

                // Request a token from Stripe:
                console.log(event.data);
                let token =   Stripe.card.createToken($form, event.data.stripeResponseHandler);
        ...
        buy.addEventListener("click",collectDataMiddleGuy(),false);
